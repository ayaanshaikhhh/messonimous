...
export async function POST(request:Request) {
    ...
    try {
        ...
        if(FindVerificationCode.verificationCodeExpiry < Date.now()){
            ...
            return Response.json( { success: false, message: "OTP has expired. Please request a new OTP.", }, { status: 400 } 
            ); }
            ...
            await AccountDeletionVerification.deleteOne({ _id: FindVerificationCode._id, });

            return Response.json( 
                { success: true,
                message: "Your account has been scheduled for deletion. You have 7 days to recover it.", deleteDate, }, 
                { status: 200 })
